...
term <- fread('~/490R Files/g_us_term_of_grant_2012_2021.csv')

# Read data
starttime <- Sys.time()
# patents file (grant date)
patent <- fread('~/490R Files/g_patent_2012_2021.csv')
# disambiguated assignee (company name)
assignee <- fread('~/490R Files/g_assignee_disambiguated_2012_2021.csv')
# term grant (expiration date)
location <- fread('~/490R Files/g_location_disambiguated_2012_2021.csv')
# cpc codes (for heatmap)
cpc <- fread('~/490R Files/g_cpc_current_2012_2021.csv')
Sys.time() - starttime
...
dt <- cpc %>% filter(grepl(pattern = 'Y02T',x = cpc$cpc_group,ignore.case = T))
# merge with patents
...
segments_names <- c('road','railways','aeronautics','maritime')
segments_codes <- c('Y02T10/','Y02T30/','Y02T50/','Y02T70/')
...
# Save file
write.csv(totals,'~/490R Files/competitive positioning.csv',row.names = F)