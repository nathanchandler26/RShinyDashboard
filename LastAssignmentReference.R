...
keep <- cpc %>% 
  filter(grepl(pattern = 'H04R|H04S',x = cpc$cpc_group,ignore.case = T)) %>% 
  select(patent_id) %>% 
  unique()
...
abstracts$patent_abstract <- str_replace_all(string = abstracts$patent_abstract,pattern = '�|�|�|�',replacement = '')
